...
# LGB base (leaves=63) × 3시드 — n_estimators=5000, early_stopping=120
...
# LGB deep (leaves=95) × 2시드 — lr=0.025 (기존 파이프라인 동일)
...
sub_v17.to_csv(os.path.join(NPY_DIR, f"v17b_alone_oof{opt_auc:.5f}_{ts}.csv"), index=False)
...
sub_blend.to_csv(os.path.join(NPY_DIR, f"v17b_blend_oof{blend_auc:.5f}_{ts}.csv"), index=False)

# numpy 저장 (추가 블렌딩용)
np.save(os.path.join(NPY_DIR, f"oof_v17b_auc_{opt_auc:.5f}.npy"), oof_pool["v17"])
np.save(os.path.join(NPY_DIR, f"test_v17b_auc_{opt_auc:.5f}.npy"), test_pool["v17"])

print(f"\n{'='*55}")
print(f"[A] v17b 단독  : v17b_alone_oof{opt_auc:.5f}_{ts}.csv")
print(f"[B] 최종블렌딩 : v17b_blend_oof{blend_auc:.5f}_{ts}.csv  ← 제출 권장")
print(f"{'='*55}")
